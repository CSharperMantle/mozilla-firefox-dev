...
class ConnectionEntry : public SupportsWeakPtr {
 public:
  ...
  // Describes the origin, not necessarily the entry's current connections.
  // Under Happy Eyeballs an Alt-Svc alternate shares the origin's entry, so an
  // h2-origin entry can hold an h3 connection while this still reports
  // IsHttp3()==false. Don't infer protocol/route from it; query the live
  // connections (HasActiveH3Connection(), GetH2orH3ActiveConn()). Replaced
  // wholesale when a property changes, since the conn info is immutable.
